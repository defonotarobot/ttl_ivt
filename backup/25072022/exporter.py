    ...
    def export(self, env: str, env_naming: str, use_full_table_list: bool):
        ...
        while connected_flag == 0:
            ...
            try:
                ...
                print(color.RED + "Error connecting to " + color.BOLD + "{0}".format(vpn_is) + color.END + color.RED + " VPN" + color.END)
                tryagain = "x"
                while tryagain not in "yn":
                    tryagain = str(input(color.BOLD + color.RED + "ACTION REQUIRED: " + color.END + "Would you like to try again? [y/n] (Check VPN first maybe?)"))
                if tryagain.lower() == "n":
                    break
            ...
            for table_name in table_list:
                ...
                while try_again_flag == 1:
                    try:
                        ...
                        print(color.RED + "Error occurred querying for the table {0}. - "+ color.BOLD + "{1}".format(table_name, error) + color.END + "\n")
                        tryagain = "x"
                        while tryagain not in "yn":
                            tryagain = str(input(color.BOLD + color.RED + "ACTION REQUIRED: " + color.END + "Would you like to try again? [y/n] (Check VPN first maybe?)"))
                        if tryagain.lower() == "n":
